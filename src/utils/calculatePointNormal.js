export default function calculatePointNormal(pointInput, normalInput) {
  ...
  if (point && normal) {
    ...
    if (point.length === 2 && normal.length === 2) {
      var pointNormalForm = `${parseFloat(normal[0]).toFixed(
        2
      )}(x - ${parseFloat(point[0]).toFixed(2)}) + ${parseFloat(
        normal[1]
      ).toFixed(2)}(y - ${parseFloat(point[1]).toFixed(2)}) = 0`;
      ...
      var pointNormalForm = `(${parseFloat(normal[0]).toFixed(
        ...
        normal[1]
      ).toFixed(2)})(y - ${parseFloat(point[1]).toFixed(2)}) + (${parseFloat(
        normal[2]
      ).toFixed(2)})(z - ${parseFloat(point[2]).toFixed(2)}) = 0`;
      var generalForm = `${parseFloat(normal[0]).toFixed(2)}x + ${parseFloat(
        normal[1]
